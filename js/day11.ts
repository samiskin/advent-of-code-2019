//---------------------------------------------------------------------------------
import { _, eq, neq, gt, lt, range, range2, nRange, print, printGrid, createProgram, runProgram, hash } from "./utils";
console.clear();
console.log("\n");
//---------------------------------------------------------------------------------
...
const prog = createProgram(input);

const map = {};
map[hash(coord)] = 1;

while (true) {
  const { type } = runProgram(prog);
  if (type === 'halt') break;
  const color = map[hash(coord)] ? 1 : 0;
  prog.inputQueue.push(color);
  const { value: col } = runProgram(prog);
  if (col === Cols.WHITE) {
    map[hash(coord)] = 1;
  } else {
    map[hash(coord)] = 0;
  }
  const { value: nextDir } = runProgram(prog);
  if (nextDir == 0) {
    dir = (dir + 4 - 1) % 4;
  } else {
    dir = (dir + 4 + 1) % 4;
