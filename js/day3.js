...
const getPoints = cmds =>
  cmds.reduce(
    (points, cmd) => {
      let [dir, ...rest] = cmd.split("");
      const num = parseInt(rest.join(""));
      for (let i = 1; i <= num; i++) {
        const [x, y] = points[points.length - 1];
        const [dx, dy] = getStep(dir);
...
const hash = ([x, y]) => `${x} ${y}`;
const [pA, pB] = [a, b].map(getPoints);
const [aMap, bMap] = [pA, pB].map(ps =>
  ps.reduce((map, [x, y], i) => {
    const key = hash([x, y]);
...
const setB = new Set(pB.map(hash));
...
console.log(
  pA
    ...
    .reduce((min, [x, y]) => {
      const key = hash([x, y]);
      return setB.has(key) ? Math.min(min, aMap[key] + bMap[key]) : min;
    }, 100000)
);
