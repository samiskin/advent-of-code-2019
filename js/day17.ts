...
let startPos = null;
while (true) {
  const { type, value } = runProgram(prog);
  if (type === 'halt') break;
  if (value === 35) {
    map[hash([x, y])] = '#';
    x += 1;
...
let p1Result = getMapPoints(map)
  .filter(([x, y]) => map[hash([x, y])] == '#')
  .filter(
    ([x, y]) => getNeighbors(x, y)
    .every(([nx, ny]) => map[hash([nx, ny])] === '#')
  )
...
let path = [];
while (true) {
  const rightDir = (curDir + 4 + 1) % 4;
  const leftDir = (curDir + 4 - 1) % 4;

  const forward = dirToDiff(curDir);
  const right = dirToDiff(rightDir);
  const left = dirToDiff(leftDir);
  let next = [forward, right, left]
    .filter(([dx, dy]) => map[hash(addPos(curPos, [dx, dy]))] == '#')[0] as [number, number];
  if (!next) break;
  if (next == forward) {
    path[path.length - 1]++;
  } else if (next == right) {
  ...
  curPos = addPos(curPos, next);
}

console.log(path.join(','))


const A = 'R,6,L,12,R,6';
const B = 'L,12,R,6,L,8,L,12';
const C = 'R,12,L,10,L,10';
const mainRoutine = 'A,A,B,C,B,C,B,C,B,A';

let fullPath = 'R,6,L,12,R,6,R,6,L,12,R,6,L,12,R,6,L,8,L,12,R,12,L,10,L,10,L,12,R,6,L,8,L,12,R,12,L,10,L,10,L,12,R,6,L,8,L,12,R,12,L,10,L,10,L,12,R,6,L,8,L,12,R,6,L,12,R,6';
console.log(fullPath == [A,A,B,C,B,C,B,C,B,A].join(','))



const p2Prog = createProgram(inputStr, { '0': 2 });
console.log(p2Prog.mem[0])
...
p2Prog.inputQueue.push(
  ...inputChars.map((c) => c.charCodeAt(0))
);

let lastOutput = null;
while (true) {
  const { type, value } = runProgram(p2Prog);
  if (type === 'halt') break;
  lastOutput = value;
}
console.log("P2:", lastOutput);
